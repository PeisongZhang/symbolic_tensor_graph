...
def main():
    ...
    if args.model_type == "llama" or args.model_type == "dense":
        ...
        if os.environ.get("STAGE_MICROBATCH_OPTIMIZE", "0") == "0":
            ...
            print("[Warning] MICROBATCH OPTIMIZE sometimes generate incorrect graphs, use with caution!")
            # Shortcut path: skip per-microbatch graph replication and just
            # rename the Batch symbol. MicroBatchReplicator has NOT run, so the
            # rename is applied exactly once (matching the dense/GPT path).
